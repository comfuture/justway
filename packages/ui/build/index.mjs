...
glob("src/**/*.css", {debug: true}, async (error, files) => {
  ...
  for (const file of files) {
    fs.open(file)
      ...
      .then(([css, jss]) => {
        ...
        fs.writeFile(file.replace(/(^src)/, 'dist/css'), css)
        fs.writeFile(file.replace(/^src\/(.*)\.css$/, (match, $1) => `dist/jss/${$1}.js`), `export default ${JSON.stringify(jss)}`)
      })
  }
})